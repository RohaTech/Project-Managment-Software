...
export default function ProjectShow({ project, tasks, users, members }) {
    ...
    const assignedOptions = members.map((member) => ({
        value: member.id,
        label: member.name,
    }));
    ...
    const [editableAssigned, setEditableAssigned] = useState(
        tasks.reduce((acc, task) => {
    ...
    const handleInvite = (e) => {
        ...
        axios
            .post("http://localhost:8001/api/invite-member", { email, project:project.id })
            .then((response) => {
                setMessage(
