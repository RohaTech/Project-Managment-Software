  ...
  return (
    <tr className={`bg-gray-${100 + level * 50}`}>
            <td className="px-4 py-1 border border-l-0 border-slate-300 flex items-center group transition duration-600 ease-in-out">
                <span className="cursor-pointer" onClick={() => handleToggle(subtask.id)}>
            ...
            <td className="px-4 py-1 border border-slate-300">
                <select className="border-0" onChange={(e)=> setData('assigned', e.target.value)} onBlur={handleSubtaskSubmit}>
                    {members.map((member, index) => (
                        <option key={index} value={member.id} selected={member.id === subtask.assigned}>{member.name}</option>
                        ))} </select>
            </td>
            <td className="px-4 py-1 border border-slate-300">
                <select className="border-0" onChange={(e)=> setData('status', e.target.value)} onBlur={handleSubtaskSubmit}>
                    {statusOptions.map((status, index) => (
                        <option key={index} value={status.value} selected={status.value === subtask.status}>{status.label}</option>
                    ))}
                </select>
            </td>
            <td className="px-4 py-1 border border-slate-300">
                <select className="border-0" onChange={(e)=> setData('priority', e.target.value)} onBlur={handleSubtaskSubmit}>
                    {priorityOptions.map((priority, index)=> (
                        <option key={index} value={priority.value} selected={priority.value === subtask.priority}>{priority.label}</option>
                    ))}
                </select>
            </td>
            <td className="px-4 py-1 border border-slate-300">
