...
function PopEditProject({ openEdit, setOpenEdit, project }) {
    ...
    return (
        <Dialog open={openEdit} onClose={() => setOpenEdit(false)} className="relative z-50">
    <div className="fixed inset-0 flex w-screen items-center justify-center p-4 bg-black bg-opacity-50">
        <DialogPanel className="max-w-2xl space-y-4 bg-slate-200 p-8 rounded-lg shadow-lg w-[500px]">
            <DialogTitle className="text-xl font-bold text-gray-800">Edit Project</DialogTitle>
            <form onSubmit={submit} className="space-y-6 w-2xl">
                <div>
                    <InputLabel htmlFor="name" value="Project Title" className="text-gray-700 font-bold" />
                    <TextInput
                        id="name"
                        name="name"
                ...
                <div className="mt-4">
                    <InputLabel htmlFor="description" value="Description" className="text-gray-700" />
                    <Textarea
                        id="description"
                        name="description"
                        className="mt-1 block w-full rounded-md border-gray-300 bg-slate-200 text-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                ...
                <div className="flex items-center justify-end mt-6">
                    <PrimaryButton className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-md shadow-md transition duration-300 ease-in-out" disabled={processing}>
                        Save
                    </PrimaryButton>
                </div>
            </form>
